...
C    Output para
      DIMENSION  AIC_E(MAXNG),             !Akaike Information Criterion
     &           BIC_E(MAXNG),                      !Bayesian  "  "       "  "
...
C    Call the main control section of the program
      CALL MAIN_EMD(NIND,NATT,NG0,NG1,NCOV,X,TOLS,USA,
...
C
C
      SUBROUTINE SETUP_EMD(NIND,NATT,NG0,NG1,NCOV,X,TOLS,
...
C      questions with the user (or alternatively from a file)
     ...
     &          T(MAXNG),W(MNIND,MAXNG)
      INTEGER   IDT(MNIND)
      CHARACTER INFYLE*20,OFYLE*20,ANSWER*20,OFYLE2*20,MFYLE*20
...
CCCCCCCc      IF (FLAGS(8).LE.4) THEN
     ...
     &  .OR.(ANSWER.EQ.'y').OR.(ANSWER.EQ.'Y')) THEN
      CALL EXOPT_EMD(NG1,TOLS,XUU,NCOV,NEDRAN)
      ENDIF
      ENDIF
...
535    FORMAT (////////////////////////)
        FI=6
        CALL SUMRY_EMD(NIND,NATT,NG0,NG1,NCOV,TOLS,NEDRAN,FI)
        IF (FLAGS(8).EQ.1) THEN
...
17    CONTINUE
      ...
      IF (FLAGS(23).EQ.1) THEN
       CALL USRPARAMETERS_E(NIND,NATT,NG0,NCOV,XMU,XVAR,
     &           TT)
      ENDIF

       IF (FLAGS(24).EQ.1) THEN
        CALL USRALLOC_EMD(NIND,NATT,NG0,IDT,IER)
        IF (IER.EQ.6) RETURN
       ENDIF
...
C       Correct posterior probabilities for classified data
        CALL PARCORR_E(NIND,NATT,NG0,USA,W)
       ENDIF
...
599    RETURN
      END
       ...
       SUBROUTINE EXOPT_EMD(NG1,TOLS,XUU,NCOV,NEDRAN)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        INCLUDE 'emmix/EMMIX.max'
...
600       RETURN
      ...
      SUBROUTINE USRPARAMETERS_E(NIND,NATT,NG,NCOV,XMU,XVAR,
     &           T)
...
465   FORMAT (5X,10F7.3)
      ...
      SUBROUTINE USRALLOC_EMD(NIND,NATT,NG,IDT,IER)
...
C
C
      SUBROUTINE PRED_E(NIND,NATT,NCOV,NG,X,XMU,XVAR,V,DV,T,USA,WL,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        INCLUDE 'emmix/EMMIX.max'
      INTEGER FLAGS(40),FYLENO
      INTEGER USA(MNIND)
     ...
     &         XMAH(MNIND,MAXNG)
       IER=0
       IOUNT=1
      CALL ESTEP_EMD(NIND,NATT,NG,X,XMU,XVAR,T,WL,W,XUU,USA,DV,
     &            XLOGL,IOUNT,XMAH,IER)
      ...
      IF (IER.EQ.-111) THEN
        ...
        IER=0
      ENDIF
      CALL OUTLOOP_E(NIND,NATT,NG,XMU,DV,T,NCOV,IOUNT,XLOGL,
     &                   W,IDT,X,USA,U)
      ...
      SUBROUTINE DISC_E(NIND,NATT,NCOV,NG,X,XMU,XVAR,V,DV,T,USA,WL,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        INCLUDE 'emmix/EMMIX.max'
      INTEGER FLAGS(40),FYLENO
      INTEGER USA(MNIND)
...
C      Calculate parameter estimates
       CALL ESTIMATES_E(NIND,NATT,NG,X,IDT,WL,NCOV,XMU,V,XVAR,DV,
     &                  T,USA,IER)
       WRITE(FYLENO,*) '    Parameters from classified data'
       WRITE(FYLENO,*) '    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
C      Write parameter estimates to output file
       CALL OUTESTIMATES_E(NIND,NATT,NG,NCOV,XMU,V,XVAR,DV,T,XUU)
     ...
     &            XLOGL,IOUNT,XMAH,IER)
      ...
      IF (IER.EQ.-111) THEN
       ...
       WRITE(*,*) 'Warning : Some points have zero Likelihood'
      ENDIF
       CALL CAPART_E(NIND,NATT,NG,W,IDT,USA,XCC)
      WRITE(FYLENO,*)
      WRITE(FYLENO,*) '     *******************************'
...
1177    FORMAT (2X,10I4)
     ...
     &  XLOGL,IOUNT,XMAH,IER)
        IF (IER.GT.0) RETURN
        XTMP=1
        CALL MSTEP_EMD(NIND,NATT,NG,NCOV,X,XVAR,V,DV,
...
C
C
      SUBROUTINE MAIN_EMD(NIND,NATT,NG0,NG1,NCOV,X,TOLS,USA,
     &   SIG,XUU,XMU,XVAR,T,IDT,W
     &   ,AIC,BIC,AWE,TLL,IER)
...
C   OUTPUT PARAMETERS
      DIMENSION
     &           AIC_E(MAXNG),                      !Akaike Information Criterion
     &           BIC_E(MAXNG),                      !Bayesian  "  "       "  "
     &           AWE(MAXNG),                      !Approx. Weight. Evidence
     &           TLL(MAXNG)                       !-2log(Lambda)
...
C    Invert user supplied covariance matrices
      IF (FLAGS(23).EQ.1) THEN
        CALL INVRT_EMD(NATT,NCOV,NG0,XVAR,V,DV,IER)
      ENDIF
...
C      Discriminant rule with densities (OPTION 4)
      ELSEIF (FLAGS(8).EQ.4) THEN
       CALL DISC_E(NIND,NATT,NCOV,NG0,X,XMU,XVAR,V,DV,T,USA,WL,IER)
C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C      Prediction (OPTION 5)
      ELSEIF (FLAGS(8).EQ.5) THEN
       CALL PRED_E(NIND,NATT,NCOV,NG0,X,XMU,XVAR,V,DV,T,USA,WL,IER)
C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
c     Display the a summary of the results to determine the number of groups
      IF (FLAGS(8).EQ.3) THEN
        CALL ANASUM_E(NIND,NATT,NG0,NG1,NCOV,TXML,TLL,AIC,BIC,AWE)
      ENDIF

      RETURN
      END

      SUBROUTINE ANASUM_E(NIND,NATT,NG0,NG1,NCOV,TXML,TLL,AIC
     &                  ,BIC,AWE)
c
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        INCLUDE 'emmix/EMMIX.max'
     ...
     &           AIC_E(MAXNG),BIC_E(MAXNG),AWE(MAXNG)
     ...
     &  '-----------------------------'
      ...
      DO 59 NG=NG0,NG1
        ING=ING+1
C        CALL CRITERIA_EMD(NG,TXML(ING),NIND,NATT,NCOV,AIC_E(ING),BIC_E(ING),
C     &                AWE(ING))
        IF (FLAGS(10).EQ.0) THEN
...
66          FORMAT(' |',I3,' | NO VAL   | NO VAL   | NO VAL   |',
     &      ' NO VAL   | NO VAL   |')
          ELSE
            WRITE(FYLENO,58) NG,TXML(ING),TLL(ING),AIC_E(ING),BIC_E(ING)
          ENDIF
          WRITE(FYLENO,*)'----------------------------------------',
     &    '---------------------'
        ELSE
...
67          FORMAT(' |',I3,' | NO VAL   | NO VAL   | NO VAL   |',
     &      ' NO VAL   | NO VAL    |')
          ELSEIF (ING.EQ.1) THEN
            WRITE(FYLENO,68) NG,TXML(ING),AIC_E(ING),BIC_E(ING)
...
59      CONTINUE
      ...
      SUBROUTINE SUMRY_EMD(NIND,NATT,NG0,NG1,NCOV,TOLS,NEDRAN,FI)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        INCLUDE 'emmix/EMMIX.max'
...
599    RETURN
      END
        ...
        SUBROUTINE INPSUM_EMD(NIND,NATT,NG)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        INCLUDE 'emmix/EMMIX.max'
      INTEGER FLAGS(40),FYLENO
      COMMON /STORE1/ SEED,RANDTYPE,IX,IY,IZ
     ...
     &           NG,'Posterior probabilities on each line'
      ...
      SUBROUTINE INVRT_EMD(NATT,NCOV,NG,XVAR,V,DV,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        INCLUDE 'emmix/EMMIX.max'
      INTEGER FLAGS(40),FYLENO
      COMMON /STORE2/ FLAGS,FYLENO
      DIMENSION XVAR(MAXNG,MNATT),DV(MAXNG)
         ...
         IF ((NCOV.EQ.3).OR.(NCOV.EQ.5)) THEN
          CALL GDET_EMD(NCOV,NATT,NG,XVAR,V,DV,IER)
          DO 5401 K=2,NG
           DV(K)=DV(1)
            DO 5401 I=1,NATT
              XVAR(K,I)=XVAR(1,I)
...
C     Written by D.Peel Oct 1994 based on program pkmmgen 1992
      ...
      SUBROUTINE AUTOPARTITION_E(NIND,NATT,NG,NCOV,X,
...
15       FORMAT (2X,'------------------------------------'
     &                ,'------------------------------------')
        IF (HIRFLG.EQ.1) THEN
         CALL HIERCON_E(NIND,NATT,NG,NCOV,X,WL,WBEST,XMLBEST,XML,NS,
     &        MAXNS,TOLS,NAMBEST,XUU,XUUBEST,USA,CORIND,FFG,IER)
        ...
        IF (FLAGS(2).EQ.1) THEN
         ...
         IZ=FSEED(3)
        ENDIF
        CALL RANDST_E(NIND,NATT,NG,NCOV,X,WL,WBEST,XMLBEST,XML,
     &    NS,MAXNS,TOLS,NAMBEST,XUU,XUUBEST,USA,CORIND,FFG,IER)
        IF (FLAGS(11).EQ.0) WRITE(FYLENO,15)
        CALL KMEANCON_EMD(NIND,NATT,NG,NCOV,X,WL,WBEST,XMLBEST,XML,NS,
     &        MAXNS,TOLS,NAMBEST,XUU,XUUBEST,USA,CORIND,FFG,IER)
        WRITE(FYLENO,15)
...
C     &	' starting grouping',/,'can be stored and listed. All',
...
50     CONTINUE
       ...
       SUBROUTINE RANDST_E(NIND,NATT,NG,NCOV,X,WL,WBEST,XMLBEST,
...
C           assignment IDT
c          FLAGS(9)=3
          CALL FIT_EMD(NIND,NATT,NG,NCOV,X,WL,XUU,XUUBEST,USA,
...
199     CONTINUE
       ...
       SUBROUTINE HIERCON_E(NIND,NATT,NG,NCOV,X,WL,WBEST,XMLBEST,
     &    XML,NS,MAXNS,TOLS,NAMBEST,XUU,XUUBEST,USA,CORIND,FFG,IER)
...
c          Determine Hierarchical grouping
           CALL HIE_E(NIND,NATT,NG,X,IDT,ISU(JJ),IS(JJ),BETA(JJ),IFAULT)
...
260      RETURN
      ...
      SUBROUTINE KMEANCON_EMD(NIND,NATT,NG,NCOV,X,WL,WBEST,XMLBEST,XML,
     &      NS,MAXNS,TOLS,NAMBEST,XUU,XUUBEST,USA,CORIND,FFG,IER)
...
305     FORMAT(2X,'           K-means Starting allocation',I3,/,
     &         2X,'           ~~~~~~~ ~~~~~~~~ ~~~~~~~~~~~')
        ...
        IF (FLAGS(3).EQ.1) THEN
         ...
         ENDIF
        ENDIF
        CALL KMEANS_EMD(NIND,NATT,NG,X,IDT,EPSILON,IER)
        IF (IER.EQ.-41) THEN
...
316    CONTINUE
      ...
      END
       ...
       SUBROUTINE CHECK_EMD(NCOV,NG,NIND,NATT,IDT,SWAPID)
...
420       CONTINUE
          DO 430 IF1=1,NG
           IF(NO(IF1).LT.IRANGE) THEN
             CALL SWAP_E(IDT,NO,NIND,NATT,NG,IF1)
             SWAPID=IF1
           ENDIF
430       CONTINUE
         ENDIF
       ...
       SUBROUTINE SWAP_E(IDT,NO,NIND,NATT,NG,I)
...
510     CONTINUE
      ...
      SUBROUTINE FIT_EMD(NIND,NATT,NG,NCOV,X,WL,XUU,XUUBEST,USA,
     &    IDT,WBEST,XML,XMLBEST,NS,MAXNS,TOLS,CORIND,FFG,IER)
...
C     partition contained in IDT
     ...
     &           XUUBEST(MAXNG),TXUU(MAXNG)
           IER=0
           SWAPID=0
           CALL CHECK_EMD(NCOV,NG,NIND,NATT,IDT,SWAPID)
	   IF (SWAPID.GT.0) THEN
...
C	write (*,*) (IDT(III),III=1,NIND)
      CALL ESTIMATES_E(NIND,NATT,NG,X,IDT,WL,NCOV,XMU,V,
     &	 XVAR,DV,T,USA,IER)
...
10101     CONTINUE
          ...
          CALL  TMOM_E(NIND,NATT,NG,X,XMU,XVAR,W,TXUU)
        ELSE
         DO 1999 II=1,NG
          TXUU(II)=XUU(II)
1999     CONTINUE
	 ...
	 CALL LOOP_E(NIND,NATT,NG,X,XMU,V,XVAR,DV,T,NCOV,IER,TXML,
...
C
C
       SUBROUTINE NMM_E(NIND,NATT,NG,NCOV,IDT,W,X,WL,
...
C	NCOV=1
C       Calculate parameter estimates
        CALL ESTIMATES_E(NIND,NATT,NG,X,IDT,WL,NCOV,XMU,V,XVAR,
     &               DV,T,USA,IER)
       IOUNT=1
       CALL ESTEP_EMD(NIND,NATT,NG,X,XMU,XVAR,T,WL,W,XUU,USA,DV,
     &            XLOGL,IOUNT,XMAH,IER)
...
C       Generate maximum likelihood solution from various starts
	CALL AUTOPARTITION_E(NIND,NATT,NG,NCOV,X,
     &	 W,WL,TOLS,XUU,USA,FSEED,IER)
	IF (IER.EQ.2) GOTO 60
C       Correct posterior probabilities for classified data
        CALL PARCORR_E(NIND,NATT,NG,USA,W)
C       Calculate parameters from posterior probabilities
        TEMP=0
        CALL MSTEP_EMD(NIND,NATT,NG,NCOV,X,XVAR,V,
...
23     CONTINUE
      CALL FINOUT_E(NIND,NATT,NCOV,NG,XUU,XMU,XVAR,T,IDT,XCC)
...
99    RETURN
      ...
      SUBROUTINE FINOUT_E(NIND,NATT,NCOV,NG,XUU,XMU,XVAR,T,IDT,XCC)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        INCLUDE 'emmix/EMMIX.max'
      INTEGER FLAGS(40),FYLENO
      COMMON /STORE2/ FLAGS,FYLENO
...
99    RETURN
      ...
      SUBROUTINE ESTEP_EMD(NIND,NATT,NG,X,XMU,XVAR,T,WL,W,XUU,USA,DV,
     &                 XLOGL,IOUNT,XMAH,IER)
...
C         Calculate Log-Likelihood contribution from point JJ
     ...
     & (LOG(TG)-0.5*AG-0.5*(DG)-(FLOAT(NATT)/2.0)*LOG(2.0*PI))
      ...
      ENDIF
950   CONTINUE
      write(*,*)IOUNT,' lik= ',XLOGL(IOUNT),
     &                  LOOK/FLOAT(NIND)*100.0,'%'
      RETURN
      END

      SUBROUTINE MSTEP_EMD(NIND,NATT,NG,NCOV,X,XVAR,V,
     &                 DV,XMU,WTOT,T,W,XUU,XMAH,TEMP,U,IER)
...
1370    CONTINUE
        IF (FLAGS(7).GT.1) THEN
          CALL TFREE_E(NIND,NATT,NG,XUU,U,W,1,IER)
        ENDIF
      ELSE
...
c             for un-inverted matrix
         CALL LCAL_EMD(NIND,NATT,NG,X,W,XMU,WSUM,WUSUM,XVAR,U)
C     Test if a common covariance matrix is specified (NCOV = 1)
...
C       Obtain inverse of this matrix
        CALL GDET_EMD(NCOV,NATT,1,XVAR,V,DV,IER)
...
1035  FORMAT (/2X,'Note: Because of problems with SYMINV this program',
     ...
     &       'New means but old covariance matrices are printed.')
      ...
      SUBROUTINE PARCORR_E(NIND,NATT,NG,USA,W)
...
60          CONTINUE
      ...
      SUBROUTINE ESTIMATES_E(NIND,NATT,NG,X,IDT,WL,NCOV,
...
C       matrix
	...
	NIND=NTNIND
      ...
      SUBROUTINE OUTESTIMATES_E(NIND,NATT,NG,NCOV,XMU,V,XVAR,DV,T,XUU)
...
237   FORMAT (5X,10F7.3)
      ...
      SUBROUTINE GDET_EMD(NCOV,NATT,NG,XVAR,V,DV,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C     for each covariance matrix in turn.
     ...
     &          DV(MAXNG)
      IF (NCOV.EQ.3) THEN
       CALL GDET3_E(NATT,XVAR,V,DV,IER)
      ELSEIF (NCOV.EQ.4) THEN
       CALL GDET4_E(NATT,NG,XVAR,V,DV,IER)
      ELSEIF (NCOV.EQ.5) THEN
       CALL GDET5_E(NATT,XVAR,V,DV,IER)
      ELSEIF (NCOV.EQ.6) THEN
       CALL GDET6_E(NATT,NG,XVAR,V,DV,IER)
      ENDIF
      RETURN
      END

      SUBROUTINE GDET3_E(NATT,XVAR,V,DV,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C     This subroutine reads all covariance matrices, then calls
C     SYMINV, which inverts a matrix and calculates its determinant,
C     for each covariance matrix in turn.
...
830   CONTINUE
      ...
      SUBROUTINE GDET5_E(NATT,XVAR,V,DV,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C     This subroutine reads all covariance matrices, then calls
C     SYMINV, which inverts a matrix and calculates its determinant,
C     for each covariance matrix in turn.
     ...
     &          DV(MAXNG)
      ...
      SUBROUTINE GDET4_E(NATT,NG,XVAR,V,DV,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C     This subroutine reads all covariance matrices, then calls
C     SYMINV, which inverts a matrix and calculates its determinant,
C     for each covariance matrix in turn.
...
830   CONTINUE
      ...
      SUBROUTINE GDET6_E(NATT,NG,XVAR,V,DV,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C     This subroutine reads all covariance matrices, then calls
C     SYMINV, which inverts a matrix and calculates its determinant,
C     for each covariance matrix in turn.
...
830   CONTINUE
      ...
      SUBROUTINE LOOP_E(NIND,NATT,NG,X,XMU,V,XVAR,DV,
...
c         W(III,IDT(III))=1
c10101    CONTINUE
c        CALL  TMOM_E(NIND,NATT,NG,X,XMU,XVAR,W,XUU)
...
c       IF (IER.GT.0) GOTO 9999
       ...
       IF (FLAGS(2).EQ.1) THEN
         CALL SEM_EMD(NIND,NATT,NG,W)
       ENDIF
C       CALL CAPART_E(NIND,NATT,NG,W,IDT,USA,XCC)
...
9999   CONTINUE
      ...
      SUBROUTINE CAPART_E(NIND,NATT,NG,W,IDT,USA,XCC)
...
1140  CONTINUE
      ...
      SUBROUTINE OUTLOOP_E(NIND,NATT,NG,XMU,DV,T,NCOV,IOUNT,XLOGL,
     &                   W,IDT,X,USA,U)
...
2000  CONTINUE
      ...
      SUBROUTINE LCAL_EMD(NIND,NATT,NG,X,W,XMU,WSUM,WUSUM,XVAR,U)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        INCLUDE 'emmix/EMMIX.max'
      INTEGER FLAGS(40),FYLENO
      COMMON /STORE2/ FLAGS,FYLENO
...
1360  CONTINUE
      ...
      SUBROUTINE CRITERIA_EMD(NG,XLGL,NIND,NATT,NCOV,AIC,BIC,AWE)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER FLAGS(40),FYLENO
      COMMON /STORE2/ FLAGS,FYLENO
C
...
C      Calculate the value of the Approximate Weight of Evidence
      ...
      SUBROUTINE SEM_EMD(NIND,NATT,NG,W)

      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        INCLUDE 'emmix/EMMIX.max'
      DIMENSION W(MNIND,MAXNG),XL(MAXNG),WSEM_EMD(MNIND,MAXNG)
      DOUBLE PRECISION RANDNUM,R
      DO 100 I=1,MNIND
       DO 100 K=1,MAXNG
	 WSEM_EMD(I,K)=0.0
...
C
C
      SUBROUTINE DOGLEG_E(N,R,LR,DIAG,QTB,DELTA,X,WA1,WA2)
      INTEGER N,LR
       INCLUDE 'emmix/EMMIX.max'
...
C
C       SUBROUTINE DOGLEG_E(N,R,LR,DIAG,QTB,DELTA,X,WA1,WA2)
C
C     WHERE
C
...
C
      ...
      SUBROUTINE FDJAC1_E(FCN,N,X,FVEC,FJAC,LDFJAC,IFLAG,ML,MU,EPSFCN,
     *                  WA1,WA2)
       INCLUDE 'emmix/EMMIX.max'
...
C     THE SUBROUTINE STATEMENT IS
C
C       SUBROUTINE FDJAC1_E(FCN,N,X,FVEC,FJAC,LDFJAC,IFLAG,ML,MU,EPSFCN,
C                         WA1,WA2)
C
C     WHERE
C
...
C
      END
      SUBROUTINE HYBRD_E(FCN,N,X,FVEC,XTOL,MAXFEV,ML,MU,EPSFCN,DIAG,
...
C     THE SUBROUTINE STATEMENT IS
C
C       SUBROUTINE HYBRD_E(FCN,N,X,FVEC,XTOL,MAXFEV,ML,MU,EPSFCN,
...
C
         IFLAG = 2
         CALL FDJAC1_E(FCN,N,X,FVEC,FJAC,LDFJAC,IFLAG,ML,MU,EPSFCN,WA1,
     *               WA2)
         NFEV = NFEV + MSUM
         IF (IFLAG .LT. 0) GO TO 300
C
...
C
         CALL QRFAC_E(N,N,FJAC,LDFJAC,.FALSE.,IWA,1,WA1,WA2,WA3)
C
...
C
         CALL QFORM_E(N,N,FJAC,LDFJAC,WA1)
C
...
C
            CALL DOGLEG_E(N,R,LR,DIAG,QTF,DELTA,WA1,WA2,WA3)
C
...
C
            CALL R1UPDT_E(N,N,R,LR,WA1,WA2,WA3,SING)
            CALL R1MPYQ_E(N,N,FJAC,LDFJAC,WA2,WA3)
            CALL R1MPYQ_E(1,N,QTF,1,WA2,WA3)
C
...
C
      END
      SUBROUTINE HYBRD1_E(FCN,N,X,FVEC,TOL,INFO,WA,LWA)
       INCLUDE 'emmix/EMMIX.max'
      INTEGER N,INFO,LWA
...
C     THE SUBROUTINE STATEMENT IS
C
C       SUBROUTINE HYBRD1_E(FCN,N,X,FVEC,TOL,INFO,WA,LWA)
C
C     WHERE
C
C       FCN IS THE NAME OF THE USER-SUPPLIED SUBROUTINE WHICH
...
c     DO 10 J = 1, N
   ...
   10    CONTINUE
      ...
      LR = (N*(N + 1))/2
      INDEX = 6*N + LR
      CALL HYBRD_E(FCN,N,X,FVEC,XTOL,MAXFEV,ML,MU,EPSFCN,WA(1),MODE,
...
C
      END
      SUBROUTINE QFORM_E(M,N,Q,LDQ,WA)
       INCLUDE 'emmix/EMMIX.max'
      INTEGER M,N,LDQ
...
C     THE SUBROUTINE STATEMENT IS
C
C       SUBROUTINE QFORM_E(M,N,Q,LDQ,WA)
C
C     WHERE
C
...
C
      ...
      SUBROUTINE QRFAC_E(M,N,A,LDA,PIVOT,IPVT,LIPVT,RDIAG,ACNORM,WA)
       INCLUDE 'emmix/EMMIX.max'
...
C     THE SUBROUTINE STATEMENT IS
C
C       SUBROUTINE QRFAC_E(M,N,A,LDA,PIVOT,IPVT,LIPVT,RDIAG,ACNORM,WA)
C
C     WHERE
C
C       M IS A POSITIVE INTEGER INPUT VARIABLE SET TO THE NUMBER
...
C
      ...
      SUBROUTINE R1MPYQ_E(M,N,A,LDA,V,W)
       INCLUDE 'emmix/EMMIX.max'
      INTEGER M,N,LDA
...
C     THE SUBROUTINE STATEMENT IS
C
C       SUBROUTINE R1MPYQ_E(M,N,A,LDA,V,W)
C
C     WHERE
C
C       M IS A POSITIVE INTEGER INPUT VARIABLE SET TO THE NUMBER
...
C
      END
      SUBROUTINE R1UPDT_E(M,N,S,LS,U,V,W,SING)
       INCLUDE 'emmix/EMMIX.max'
      INTEGER M,N,LS
      LOGICAL SING
...
C     THE SUBROUTINE STATEMENT IS
C
C       SUBROUTINE R1UPDT_E(M,N,S,LS,U,V,W,SING)
C
C     WHERE
C
C       M IS A POSITIVE INTEGER INPUT VARIABLE SET TO THE NUMBER
...
C  Implemented by David Peel May 1994
      ...
      SUBROUTINE KMEANS_EMD(NIND,NATT,NG,X,IDT,EPSILON,IER)
C      Main subroutine
     ...
     &           XSTAN(MNIND,MNATT)
      IER=0
      CALL KSTAND_EMD(NIND,NATT,X,XSTAN)
      CALL KSEED_EMD(NIND,NATT,NG,XSTAN,XK,IER)
      DO 30 T=1,MKMEAN
      DO 430 KK=1,NG
        DO 420 LL=1,NATT
...
430     CONTINUE
	DO 20 KK=1,NIND
          CALL WINNER_EMD(NATT,NG,XSTAN,KK,XK,IDT,IER)
20      CONTINUE
          CALL UPDATE_EMD(NIND,NATT,NG,XSTAN,XK,IDT,IER)
          ET=RULE_EMD(NG,NATT,XKOLD,XK)
...
99    RETURN
      ...
      SUBROUTINE KSTAND_EMD(NIND,NATT,X,XNEW)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
       INCLUDE 'emmix/EMMIX.max'
...
220   CONTINUE
      ...
      SUBROUTINE KSEED_EMD(NIND,NATT,NG,XSTAN,XK,IER)
...
210   CONTINUE
      ...
      SUBROUTINE WINNER_EMD(NATT,NG,XSTAN,KK,XK,IDT,IER)
...
310   CONTINUE
      ...
      SUBROUTINE UPDATE_EMD(NIND,NATT,NG,XSTAN,XK,IDT,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        INCLUDE 'emmix/EMMIX.max'
...
499     CONTINUE
      ...
      FUNCTION RULE_EMD(NG,NATT,XKOLD,XK)
...
C
      ...
      END IF
C
C     Reduce to DIGAMA_E(X + N) where (X + N) >= C
C
    1 IF (Y .GE. C) GO TO 2
      DIGAMA = DIGAMA - ONE/Y
      Y = Y + ONE
...
C
    2 R = ONE / Y
      ...
      SUBROUTINE TMOM_E(NIND,NATT,NG,X,XMU,XVAR,W,XUU)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
       INCLUDE 'emmix/EMMIX.max'
...
150   CONTINUE
      ...
      SUBROUTINE TEQ0_E(NDUMMY,XUU1,FVEC1,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
       INCLUDE 'emmix/EMMIX.max'
      ...
      DO 110 K=1,NGTEMP
        DO 100 I=1,TNIND
          IF (XUU1.LE.0.00001)THEN
           ...
           ENDIF
          IER=0
          DIG1=DIGAMA_E(0.5*XUU1,IER)
          IF (IER.GT.0) RETURN
          DIG2=DIGAMA_E(0.5*(XUUOLD(1)+TNATT),IER)
          IF (IER.GT.0) RETURN
          XLOG1=LOG(0.5*XUU1)
...
110   CONTINUE
      ...
      SUBROUTINE TEQ_E(NG,XUU,FVEC,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
       INCLUDE 'emmix/EMMIX.max'
      COMMON /HYBD/ UTEMP(MNIND,MAXNG),TNIND,TNATT,XUUOLD(MAXNG),
     &   WTEMP(MNIND,MAXNG),NGTEMP
      ...
      DO 110 K=1,NG
        ...
        DO 100 I=1,TNIND
          ...
          ENDIF
          IER=0
          DIG1=DIGAMA_E(0.5*XUU(K),IER)
          IF (IER.GT.0) RETURN
          DIG2=DIGAMA_E(0.5*(XUUOLD(K)+TNATT),IER)
          IF (IER.GT.0) RETURN
          XLOG1=LOG(0.5*XUU(K))
...
110   CONTINUE
      ...
      END
        ...
        SUBROUTINE TFREE_E(NIND,NATT,NG,XUU,U,W,ITER,IER)
        IMPLICIT DOUBLE PRECISION (A-H,O-Z)
         INCLUDE 'emmix/EMMIX.max'
      COMMON /HYBD/ UTEMP(MNIND,MAXNG),TNIND,TNATT,XUUOLD(MAXNG),
     &    WTEMP(MNIND,MAXNG),NGTEMP
...
C	  tmp=(group-1)*3+1
	  ...
	  sizehy=(NG*(3*NG+13))/2
          ...
          XUU1=XUU(1)
	  call HYBRD1_E(TEQ0,1,XUU1,FVEC1,xtol,ifail,work,sizehy)
      DO 111 K=1,NG
       XUU(K)=XUU1
111   CONTINUE
          ELSE
	  call HYBRD1_E(TEQ,NG,XUU,FVEC,xtol,ifail,work,sizehy)
          ENDIF
C20     continue
      RETURN
      END
...
C   how this program calls the random number generator ( R=RANDNUM_EMD(SEED)
C

C   D.Peel Nov 1995

       SUBROUTINE DETERRANDOM_EMD(IER)
...
C          then it should be used in place of this function.
      ...
      DOUBLE PRECISION FUNCTION RANDOM_EMD(IX,IY,IZ)
C
...
C These subroutines will most probably be replaced in future versions.
      ...
      SUBROUTINE HIE_E(NIND,NATT,NG,X,IDT,ISU,IS,BETA,IFAULT)
...
C         'Incremental Sum of Squares (WARD''S Method) =7
      ...
      DO 10 I=1,N3
10      Z(I)=0.0
15    CALL RCLUH_E(Z(N1),Z(N2),H1,H2,H3,HH1,HH2,FORM1,NIND,NATT,
     &           ISU,FAC,X)
      ND=(NIND-1)*NIND/2
      N3=N2+ND
      N4=N3+NIND
      ...
      DO 20 I=N2,N3
20      Z(I)=0.0
      CALL DIST_E(Z(N1),Z(N2),NIND,NATT,FORM3,HD,HH4)
cccccccccccccc
c      CALL WSIM_E(Z(N2),Z(N3),NIND)
ccccccccccccccc
      DO 30 I=N3,N6
30      Z(I)=0.0

      CALL AGHICL_E(Z(N2),Z(N3),Z(N4),Z(N5),NIND,BETA,IS)
      CALL ALLOC_E(Z(N5),NIND,NATT,NG,ISU,IS,BETA,H2,HS,IDT)
      RETURN
...
50    FORMAT(1X,'THE DIMENSIONS OF THE Z ARRAY ARE TOO SMALL'/
     &       1X,'THEY SHOULD BE INCREASED FROM',I6,'TO ',I6)
      ...
      SUBROUTINE RCLUH_E(RC,TEMP,H1,H2,H3,HH1,HH2,FORM1,NIND,NATT,
     &                 ISU,FAC,X)
...
1001      FORMAT(10F10.3)
      ...
      SUBROUTINE STAND_E(A,NIND,NATT)
...
320   CONTINUE
      ...
      SUBROUTINE DIST_E(RC,DIST1,NIND,NATT,FORM3,HD,HH4)
...
400     FORM3(I)=HH4(I)
      DO 410 I=1,6
        ...
        FORM3(I1)=HD(I)
410   CONTINUE
      CALL SED_E(RC,DIST1,NIND,NATT)
      RETURN
      END


      SUBROUTINE SED_E(RC,DIST1,NIND,NATT)
...
510    CONTINUE
      ...
      SUBROUTINE AGHICL_E(D,N,M,MEMB,NT,BET,IS)
...
600   CONTINUE
      ...
      IF=NT-1
      DO 610 I=1,IF
        CALL MIND_E(D,N,IMIN,JMIN,DMIN,NT)
        M1=(I-1)*2+1
        M2=M1+1
        MEMB(M1)=M(IMIN)
        MEMB(M2)=M(JMIN)
        NTI=NT+I
        CALL DISTO_E(D,N,IMIN,JMIN,NT,BET,IS)
...
610    CONTINUE
      ...
      SUBROUTINE MIND_E(D,N,IMIN,JMIN,DMIN,NT)
...
700   CONTINUE
      ...
      SUBROUTINE DISTO_E(D,N,IMIN,JMIN,NT,BET,IS)
...
C      IS ; Code for strategy of clustering.
      ...
      DO 800 K=1,NT
        IF ((N(K).NE.0).AND.(K.NE.IMIN).AND.(K.NE.JMIN)) THEN
          K1=K
          CALL DISTQ_E(D,N,IMIN,JMIN,DISS,K1,BET,IS)
          IF(IMIN.GT.K) THEN
...
800   CONTINUE
      ...
      SUBROUTINE DISTQ_E(D,N,IMIN,JMIN,DISS,K,BET,IS)
...
10      DISS=ALA*DIK+ALB*DJK+BETA*DIJ+GAM*DIKJK
      ...
      SUBROUTINE ALLOC_E(IHIE,NIND,NATT,NG,ISU,IS,BETA,H2,HS,IDT)
...
910    HEAD(I+3)=HS(IS,I)
      IF (IS.NE.6) BETA=0.0
      CALL FUSE_E(NAME,NUMB,MEMB,IHIE,NIND,NIND,NG)
      IG1=0
      DO 920 IF=1,NG
        DO 920 IG=1,NUMB(IF)
           IG1=IG1+1
           ...
           IDT(IG2)=IF
920   CONTINUE
c      CALL CHECK_EMD(NG,NIND,IDT)
      RETURN
      END

      SUBROUTINE FUSE_E(NAME,NUMB,MEMB,IHIE,NIND,NGPS,NGPF)
...
1000  CONTINUE
      ...
      DO 1020 I=NGPS,NGPF+1,-1
        ...
        IG1=IHIE(IP)
        IG2=IHIE(IP+1)
        CALL FUSEL_E(NAME,NUMB,MEMB,IG1,IG2,NGP,NAGPN,NIND)
        NGP=NGP-1
        NF=NF+1
1020  CONTINUE
      RETURN
      ...
      SUBROUTINE FUSEL_E(NAME,NUMB,MEMB,IG1,IG2,NGL,NEWG,NIND)
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C     Memb array
      CALL SHIFT_E(MEMB,NB1,NS1,NIND)
      CALL SHIFT_E(MEMB,NB2,NS2,NIND)
      RETURN
      END


      SUBROUTINE SHIFT_E(MEMB,NB,NS,NG)
