...
mcfa<- function(Y, g, q, itmax=50, nkmeans=20, nrandom=20, tol=1.e-5,
  ...
  n <- nrow(Y)
  if(p < q)
  stop("the number of factors must not be greater than the number of variables")
  ...
  for (ii in 1:maxinit) {
    ...
    estModel <- est.mcfa(Y, g, q, itmax, tol, startModel$pivec, startModel$A,
                        startModel$xi, startModel$omega, startModel$D, convMeas)
    if((class(estModel) == "mcfa")){
      if (estModel$logL > maxLOGL){
        Hmodel <- estModel
        maxLOGL <- Hmodel$logL
      }
    cat(sprintf("g = %i, q = %i, initialization %i logL %8.4f,  maxlogL = %8.4f \n",
  ...
  Hmodel <- append(Hmodel, do.call('factor.scores.mcfa', c(list(Y=Y), Hmodel)))
...
est.mcfa <- function(Y, g, q, itmax, tol, pivec, A, xi, omega, D, convMeas, ...){
  if(!is.matrix(Y)) Y <- as.matrix(Y)
...
loglike.mcfa <- function(Y, g, q, pivec, A, xi, omega, D, ...){
  ...
  for(i in 1:g){
    ...
    if(class(InvS) == "try-error") {
      return(loglike <- paste('ill-conditioned or singular Sigma_', i, sep=''))
    }
...
factor.scores.mcfa <- function(Y, g, q, pivec, A, xi, omega, D,
  ...
  for (i in 1:g){
    ...
    U[,, i] <- U[,, i] + (Y - t(replicate(n, A%*% xi[, i, drop=F], "matrix"))) %*%
      as.matrix(gamma[,, i])
  }
