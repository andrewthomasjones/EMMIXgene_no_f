...
C    Check to see if the random number generator works (try two formats)
      CALL DETERRANDOM_F1(IER)

      CALL SETUP_F1(NIND,NATT,NCOV,NG0,NG1,NKMEANS,NRANDS,RDSB,INFYLE,
     ...
     & REV,IER)
      ...
      CALL READX_F1(NIND,NATT,X,SUB,REV)
      IF (SCAL.EQ.1) THEN
	CALL KSTAND_F1(NIND,NATT,X,XM,XVR)
        DO 220 KK=1,NIND
          DO 220 J=1,NATT
...
220     CONTINUE
      ...
      IF (START.EQ.2) THEN
        CALL READPARA_F1(NATT,NG0,XMU,XVAR,T,SUB)
      ELSEIF (START.EQ.1) THEN
        NATT=SUB(2)
        CALL READALLOC_F1(NIND,NATT,NG0,XMU,XVAR,T,X)
      ENDIF

      NATT=SUB(2)
      CALL MAIN_F1(NIND,NATT,NCOV,X,NG0,NG1,XMU,XVAR,T,MAXITS,
...
505   CONTINUE
      ...
      SUBROUTINE READX_F1(NIND,NATT,X,SUB,REV)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INCLUDE 'emmix/EMMIX-f1.max'
        REAL X(MNIND,MNATT)
        REAL TEMP(MNATT*10)
...
100	 CONTINUE
	...
	RETURN
      ...
      SUBROUTINE READPARA_F1(NATT,NG,XMU,XVAR,T,SUB)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INCLUDE 'emmix/EMMIX-f1.max'
...
400   CONTINUE
	...
	END
      ...
      SUBROUTINE READALLOC_F1(NIND,NATT,NG0,XMU,XVAR,T,X)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INCLUDE 'emmix/EMMIX-f1.max'
      REAL X(MNIND,MNATT)
      INTEGER NIND,
...
224        CONTINUE
	     ...
	     DO 227 I=1,NIND
	      ...
	      DO 226 J=1,NATT
	       ...
	       DO 226 L=1,J
	         XVAR(IDT,IC1(J,L))=XVAR(IDT,IC1(J,L))+DBLE(X(I,J)*X(I,L))
226          CONTINUE
227        CONTINUE
         DO 213 KK=1,NG0
           T(KK)=FLOAT(N(KK))/FLOAT(NIND)
             DO 213 JJ=1,NATT
              ...
              DO 213 II=1,JJ
                XVAR(KK,IC1(JJ,II))=
     &          XVAR(KK,IC1(JJ,II))-(XMU(KK,II)*XMU(KK,JJ)*FLOAT(N(KK)))
                XVAR(KK,IC1(JJ,II))=XVAR(KK,IC1(JJ,II))/FLOAT(N(KK)-1)
212       CONTINUE
213       CONTINUE
	...
	END
      ...
      SUBROUTINE MAIN_F1(NIND,NATT,NCOV,X,NG0,NG1,XMU,XVAR,T,MAXITS,
     & TOLS,AIT,OPTION,START,NRANDS,NKMEANS,FACT,RDSB,NATTQ)
...
C   INPUT PARAMETERS
	 ...
	 SETUPON=1
	 FORMOT=1
         CALL EM1(NIND,NATT,NG1,NCOV,X,XMU,XVAR,T,TXML,TOLS(2),
...
7010     CONTINUE
       ...
       IF (OPTION.EQ.2) THEN
         IF (START.EQ.3) THEN
          CALL AUTO_F1(NIND,NATT,NCOV,X,NG1,XMU,XVAR,T,MAXITS(1),
	 ...
	 FORMOT=1
         CALL EM1(NIND,NATT,NG1,NCOV,X,XMU,XVAR,T,TXML,TOLS(2),
     &          MAXITS(2),MODE,FACT,B,D,NATTQ,FORMOT,SETUPON,IER)
         CALL ESTEP_F1(NIND,NATT,NCOV,NG1,X,XMU,V,DV,T,W,
     &            XLOGL,IER)
...
8010     CONTINUE
       ...
       ELSEIF (OPTION.EQ.3) THEN
        DO 200 K=NG0,NG1
           CALL AUTO_F1(NIND,NATT,NCOV,X,K,XMU,XVAR,T,MAXITS(1),
     &      NRANDS,NKMEANS,TOLS(1),AIT,RDSB,FACT,B,D,NATTQ)
           MODE=2
	 SETUPON=0
	  FORMOT=1
           CALL EM1(NIND,NATT,K,NCOV,X,XMU,XVAR,T,TXML,TOLS(2),
     &      MAXITS(2),MODE,FACT,B,D,NATTQ,FORMOT,SETUPON,IER)
200     CONTINUE
       ENDIF
...
9010     CONTINUE
      ...
      SUBROUTINE AUTO_F1(NIND,NATT,NCOV,X,NG,XMU,XVAR,T,MAXIT,
...
C    Constants that define array sizes at compilation time.
	...
	DO I=1,NRANDS
	  ...
	  CALL RANDCON_F1(NIND,NATT,NCOV,X,NG,XMU2,XVAR2,T2
     &             ,RDSB)
	  CALL FIT_F1(NIND,NATT,NCOV,X,NG,XMU,XVAR,T,XMU2,XVAR2,T2,
...
c	  write (22,*) 'End of Random start ',I
	...
	DO I=1,NKMEANS
          ...
          write (22,*) '     Seeds ', IX,IY,IZ
	  CALL KMEANS_F1(NIND,NATT,NCOV,NG,X,XMU2,XVAR2,T2,IER)
	  CALL FIT_F1(NIND,NATT,NCOV,X,NG,XMU,XVAR,T,XMU2,XVAR2,T2,
     &	           TOL,MAXIT,AIT,XLIKE,TLIKE,TMP,FACT,B,D,NATTQ,IER)
          write(22,*)'     Log Likelihhod = ',TLIKE
	  write (22,*) '  --------------------------'
...
c	  write (22,*) 'End of K-means start ',I
	...
	END
      ...
      SUBROUTINE UPDSOL_F1(NG,NATT,NCOV,XMU,XMU2,XVAR,XVAR2,T,
...
200        CONTINUE
	...
	SUBROUTINE RANDCON_F1(NIND,NATT,NCOV,X,NG,XMU2,XVAR2,T2,
     &                   RDSB)
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C    Constants that define array sizes at compilation time.
      INCLUDE 'emmix/EMMIX-f1.max'
	...
	IF ((NCOV.EQ.1).OR.(NCOV.EQ.2)) THEN
	  CALL RANDSTD_F1(NIND,NATT,NCOV,X,NG,XMU2,XVAR2,T2,RDSB)
	ELSEIF ((NCOV.EQ.3).OR.(NCOV.EQ.4)) THEN
	  CALL RANDDIAG_F1(NIND,NATT,NCOV,X,NG,XMU2,XVAR2,T2,RDSB)
        ENDIF
        RETURN
	END

      SUBROUTINE RANDSTD_F1(NIND,NATT,NCOV,X,NG,XMU2,XVAR2,T2,
     &                   RDSB)
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C    Constants that define array sizes at compilation time.
      INCLUDE 'emmix/EMMIX-f1.max'
...
213       CONTINUE
	...
	END
      ...
      SUBROUTINE RANDDIAG_F1(NIND,NATT,NCOV,X,NG,XMU2,XVAR2,T2,RDSB)
c         Have to modify
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C    Constants that define array sizes at compilation time.
      INCLUDE 'emmix/EMMIX-f1.max'
...
112     CONTINUE
	...
	SUBROUTINE KMEANCON_F1(NIND,NATT,NCOV,X,NG,XMU2,XVAR2,T2)
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C    Constants that define array sizes at compilation time.
      INCLUDE 'emmix/EMMIX-f1.max'
	...
	END
      ...
      SUBROUTINE FIT_F1(NIND,NATT,NCOV,X,NG,XMU,XVAR,T,XMU2,XVAR2,T2,
     &               TOL,MAXIT,AIT,XLIKE,TLIKE,TMP,FACT,B,D,NATTQ,IER)
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C    Constants that define array sizes at compilation time.
      INCLUDE 'emmix/EMMIX-f1.max'
	...
	FORMOT=0
        ...
        CALL EM1(NIND,NATT,NG,NCOV,X,XMU2,XVAR2,T2,TLIKE,
	...
	IF ((TLIKE.GT.XLIKE).OR.(TMP.EQ.1.0)) THEN
         write (*,*) 'Update'
	 XLIKE=TLIKE
	 CALL UPDSOL_F1(NG,NATT,NCOV,XMU,XMU2,XVAR,XVAR2,T,T2,
	...
	END
      ...
      SUBROUTINE SETUP_F1(NIND,NATT,NCOV,NG0,NG1,NKMEANS,NRANDS,RDSB,
...
C        READ (*,*) FACT(1)
	...
	WRITE(*,*)'            Jun 18th 2001 -Add common D'
        WRITE(*,*)'      ---------------------------------'
C	CALL GETOPT_F1(OPTION)
       OPTION=2.0
       CALL GETIO_F1(INFYLE,OFYLE,IER)
	...
	write (*,*) 'you wish to output to store the model '
        READ (*,'(A)') MODFYLE
	IF (IER.NE.0) RETURN
       CALL GETNIND_F1(NIND,SUB)
       CALL GETNATT_F1(NATT,SUB)
       ...
       READ (*,*) SCAL
	CALL GETNG_F1(NG0,NG1,OPTION)
	CALL GETNCOV_F1(NCOV)
        CALL GETST_F1(START)
	IF (START.EQ.3) THEN
 	 CALL GETSRCH_F1(NRANDS,NKMEANS,RDSB)
        ENDIF
...
c	IZ=3
      ...
      SUBROUTINE GETNIND_F1(NIND,SUB)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C    Constants that define array sizes at compilation time.
      INCLUDE 'emmix/EMMIX-f1.max'
      INTEGER SUB(2+MNATT)
...
C      READ (*,*) SUB(1)
      ...
      SUBROUTINE GETNATT_F1(NATT,SUB)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C    Constants that define array sizes at compilation time.
      INCLUDE 'emmix/EMMIX-f1.max'
      INTEGER SUB(2+MNATT)
10    SUB(2)=0
      ...
      SUBROUTINE GETIO_F1(INFYLE,OFYLE,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      CHARACTER*255 INFYLE
      CHARACTER*255 OFYLE
...
519   FORMAT(
     ...
     &/,12X,'  3. Perform discriminant analysis')
      ...
      SUBROUTINE GETNG_F1(NG0,NG1,OPTION)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      IF (OPTION.EQ.3) THEN
...
510   WRITE(*,*)'Covariance matrix option (1 = equal,2 = unrestricted,'
	...
	SUBROUTINE GETSRCH_F1(NRANDS,NKMEANS,RDSB)
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
	...
	SUBROUTINE GETSEEDS_F1(IX,IY,IZ)
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
	...
	END
C
      SUBROUTINE EM1(NIND,NATT,NG,NCOV,X,XMU,XVAR,T,
...
C
     ...
     &          B(MAXNG,MNATT,MNATTQ)
      ...
      CALL FSASET2_F1(NIND,NATT,NG,NCOV,X,DV,XVAR,V,
...
900   CONTINUE
      ...
      IOUNT=IOUNT+1
      IF (FACT(1).EQ.1) THEN
       CALL ESTEP_F1(NIND,NATT,NCOV,NG,X,XMU,V,DV,T,W,
     &            XLOGL,IER)
       CALL MSTEP_F1(NIND,NATT,NG,NCOV,X,XVAR,V,DV,XMU,T,W,FACT,IER)
      ELSE
       CALL ESTEP_F1(NIND,NATT,NCOV,NG,X,XMU,V,DV,T,W,
     &            XLOGL,IER)
      ENDIF
      ...
      ENDIF
      IF (XIT_F1(IOUNT,MAXIT,TOL,TXML,XLA).EQ.1) GOTO 1099
      IF (FACT(1).EQ.1) THEN
       CALL FSAMSP_F1(NIND,NATT,NG,NCOV,XVAR,V,D,B,
...
202    FORMAT (2X,10I4)
      ...
      SUBROUTINE ESTEP_F1(NIND,NATT,NCOV,NG,X,XMU,V,DV,T,W,
     &            XLOGL,IER)
...
CCC            DO 910 J=1,NATT
            DO 905 J=1,I-1
              XJJJ=DBLE(X(JJ,J))
CCC              AL(K)=AL(K)+ALTEMP*V(K,IC1(I,J))*
CCC     &              (XJJJ-XMU(K,J))
              AL(K)=AL(K)+2.0*ALTEMP*V(K,IC1(I,J))*
     &              (XJJJ-XMU(K,J))
905         CONTINUE
	    AL(K)=AL(K)+ALTEMP*ALTEMP*V(K,IC1(I,I))
910       CONTINUE
...
CC      write (*,*) 'Det=',DV(1),DV(2)
      ...
      SUBROUTINE MSTEP_F1(NIND,NATT,NG,NCOV,X,XVAR,V,DV,
     &           XMU,T,W,FACT,IER)
...
C     Compute new estimates of mixing proportions
         CALL CALC_F1(NIND,NATT,NG,W,T,IER)
         IF (IER.NE.0) RETURN
C     Compute new estimates of group means
         CALL CALC_XM1(NIND,NATT,NG,X,W,XMU,T)
         IF (IER.NE.0) RETURN
C     Compute new estimates of covariance matrices
         CALL CALC_XVA1(NIND,NATT,NG,NCOV,X,W,XMU,T,XVAR)

      IER=0
	IF (FACT(1).EQ.0) THEN
      CALL GDET_F1(NCOV,NATT,NG,XVAR,V,DV,IER)
       IF (IER.NE.0) THEN
	...
	ENDIF
      ...
      SUBROUTINE CALC_XM1(NIND,NATT,NG,X,W,XMU,T)
...
1310  CONTINUE
      ...
      SUBROUTINE CALC_XVA1(NIND,NATT,NG,NCOV,X,W,XMU,T,XVAR)
...
20        CONTINUE
          DO 40 JJ=1,NIND
           DO 30 I=1,NATT
       XVAR(K,IC1(I,I))=XVAR(K,IC1(I,I))+(DBLE(X(JJ,I))-XMU(K,I))**2
...
160     CONTINUE
        DO 180 I=1,NATT
        XVAR(1,IC1(I,I))=XVAR(1,IC1(I,I))*FLOAT(NIND)
          DO 170 K=2,NG
        XVAR(1,IC1(I,I))=XVAR(1,IC1(I,I))+XVAR(K,IC1(I,I))*FLOAT(NIND)
170       CONTINUE
        XVAR(1,IC1(I,I))=XVAR(1,IC1(I,I))/FLOAT(NIND)
180     CONTINUE
...
220       CONTINUE
          DO 240 JJ=1,NIND
            DO 230 J=1,NATT
             DO 230 I=1,J
        XVAR(K,IC1(I,J))=XVAR(K,IC1(I,J))+(DBLE(X(JJ,I))-XMU(K,I))
...
360     CONTINUE
        ...
        DO 390 J=1,NATT
          DO 390 I=1,J
            XVAR(1,IC1(I,J))=XVAR(1,IC1(I,J))*FLOAT(NIND)
            DO 380 K=2,NG
       XVAR(1,IC1(I,J))=XVAR(1,IC1(I,J))+XVAR(K,IC1(I,J))*FLOAT(NIND)
380         CONTINUE
        XVAR(1,IC1(I,J))=XVAR(1,IC1(I,J))/NIND
390     CONTINUE
      ...
      SUBROUTINE CALC_F1(NIND,NATT,NG,W,T,IER)
...
C
      FUNCTION XIT_F1(IOUNT,MAXIT,TOL,TXML,XLA)
...
C      ENDIF
      ...
      SUBROUTINE GDET_F1(NCOV,NATT,NG,XVAR,V,DV,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C     for each covariance matrix in turn.
     ...
     &          DV(MAXNG)
      ...
      IER=0
	IF (NCOV.EQ.1) CALL GDET1_F1(NATT,NG,XVAR,V,DV,IER)
	IF (NCOV.EQ.2) CALL GDET2_F1(NATT,NG,XVAR,V,DV,IER)
	IF (NCOV.EQ.3) CALL GDET3_F1(NATT,NG,XVAR,V,DV,IER)
	IF (NCOV.EQ.4) CALL GDET4_F1(NATT,NG,XVAR,V,DV,IER)
...
c      WRITE (*,895)  K, IER
	ENDIF
      ...
      SUBROUTINE GDET1_F1(NATT,NG,XVAR,V,DV,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C     This subroutine reads all covariance matrices, then calls
C     SYMINV, which inverts a matrix and calculates its determinant,
C     for each covariance matrix in turn.
...
c         WRITE(FYLENO,*)'  Determinant is equal to zero for common cov.'
	...
	END
      ...
      SUBROUTINE GDET2_F1(NATT,NG,XVAR,V,DV,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C     This subroutine reads all covariance matrices, then calls
C     SYMINV, which inverts a matrix and calculates its determinant,
C     for each covariance matrix in turn.
...
830   CONTINUE
	...
	END
      ...
      SUBROUTINE GDET3_F1(NATT,NG,XVAR,V,DV,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C     This subroutine reads all covariance matrices, then calls
C     SYMINV, which inverts a matrix and calculates its determinant,
C     for each covariance matrix in turn.
...
830   CONTINUE
	...
	END
      ...
      SUBROUTINE GDET4_F1(NATT,NG,XVAR,V,DV,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C     This subroutine reads all covariance matrices, then calls
C     SYMINV, which inverts a matrix and calculates its determinant,
C     for each covariance matrix in turn.
...
830   CONTINUE
	...
	END
      ...
      SUBROUTINE SYMINV_F1(KNG,XVAR,N,V,NULLTY,IFAULT,RMAX,DV,
     &                  TOL,NG)
C       Modified from
...
C       Cholesky factorization of A, result in C
        CALL CHOLA_F1(KNG,XVAR,NROW,V,NULLTY,IFAULT,RMAX,W,TOL)
...
100     CONTINUE
        ...
        SUBROUTINE CHOLA_F1(KNG,XVAR,N,V,NULLTY,IFAULT,
     &                   RMAX,R,TOL)
C       Modified from
...
100     RETURN
      ...
      SUBROUTINE GDETQ_F1(NATT,XVAR,V,DV,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C     This subroutine reads all covariance matrices, then calls
C     SYMINV, which inverts a matrix and calculates its determinant,
C     for each covariance matrix in turn.
	...
	TOL=0.0
	DO 810 I=1,NATT
	  TOL=TOL+SQRT(XVAR(IC1(I,I)))
810     CONTINUE
	TOL=(TOL/NATT)*0.000001
	CALL SYMINVQ_F1(XVAR,NATT,V,NULL,IER,RMAX,DV,TOL,NG)
	IF (IER.GT.0) RETURN
	IF (NULL.NE.0) THEN
	  write (*,*) 'NULL=',NULL
815       FORMAT (/2X,'Rank deficiency of covariance matrix ',I3,
...
830   CONTINUE
      ...
      SUBROUTINE SYMINVQ_F1(XVAR,N,V,NULLTY,IFAULT,RMAX,DV,
     &                  TOL,NG)
C       Modified from
C       Algorithm AS7, Applied Statistics, Vol.17, 1968.
C
...
C       Latest revision - 18 April 1981
     ...
     &          V(MNATT*(MNATT+1)/2)
      DIMENSION W(MNATT)
        ...
        IFAULT=0
C       Cholesky factorization of A, result in C
        CALL CHOLAQ_F1(XVAR,NROW,V,NULLTY,IFAULT,RMAX,W,TOL)
        IF(IFAULT.NE.0) GO TO 100
C       Invert C & form the product (CINV)'*CINV, where CINV is the inverse
C       of C, row by row starting with the last row.
C       IROW = the row number, NDIAG = location of last element in the row.
...
100     CONTINUE
        ...
        SUBROUTINE CHOLAQ_F1(XVAR,N,V,NULLTY,IFAULT,
     &                   RMAX,R,TOL)
C       Modified from
C       Algorithm AS6, Applied Statistics, Vol.17, 1968, with
C       modifications by A.J.Miller
...
C   how this program calls the random number generator ( R=RANDNUM_F1(SEED)
C

C   D.Peel Nov 1995

       SUBROUTINE DETERRANDOM_F1(IER)
...
C          then it should be used in place of this function.
      ...
      DOUBLE PRECISION FUNCTION RANDOM_F1(IX,IY,IZ)
C
...
C  Implemented by David Peel May 1994
      ...
      SUBROUTINE KMEANS_F1(NIND,NATT,NCOV,NG,X,XMU2,XVAR2,T2,IER)
C      Main subroutine
...
1     CONTINUE
      CALL KSTAND_F1(NIND,NATT,X,XM,XVR)
      CALL KSEED_F1(NIND,NATT,NG,X,XKOLD,XM,XVR,IER)

      DO 19 KK=1,NIND
        DO 17 J=1,NATT
          XSTAN(J)=(DBLE(X(KK,J))-XM(J))/SQRT(XVR(J))
17      CONTINUE
       CALL WINNER_F1(NATT,NG,XSTAN,XKOLD,GRP,IER)
       CALL INIT_F1(NIND,NATT,NG,XSTAN,XMU2,GRP,N,IER)
       IF (IER.NE.0) RETURN
19    CONTINUE
      DO 30 T=1,MAXKM
...
300    CONTINUE
      CALL CALC_PARA_F1(NIND,NATT,NCOV,NG,X,XMU2,XKOLD,XVAR2,T2,XM,XVR)
      RETURN
      END

      SUBROUTINE CALC_PARA_F1(NIND,NATT,NCOV,NG,X,
...
400    CONTINUE
       CALL WINNER_F1(NATT,NG,XUS,XKOLD,GRP,IER)
       T2(GRP)=T2(GRP)+1.0
       DO 401 II=1,NATT
        XUS(II)=DBLE(X(KK,II))
401    CONTINUE
       DO 330 J=1,NATT
	 DO 330 I=1,J
	  XVAR2(GRP,IC1(I,J))=XVAR2(GRP,IC1(I,J))+(XUS(I)
...
410   CONTINUE
      ...
      DO 500 K=1,NG
       DO 499 I=1,NATT
        DO 499 J=1,I
	 XVAR2(K,IC1(I,J))=XVAR2(K,IC1(I,J))/T2(K)
499    CONTINUE
...
500   CONTINUE
      ...
      SUBROUTINE KSTAND_F1(NIND,NATT,X,XM,XVR)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INCLUDE 'emmix/EMMIX-f1.max'
      REAL X(MNIND,MNATT)
...
210   CONTINUE
      ...
      SUBROUTINE KSEED_F1(NIND,NATT,NG,X,XK,XM,XVR,IER)
...
210   CONTINUE
      ...
      SUBROUTINE WINNER_F1(NATT,NG,XSTAN,XK,GRP,IER)
...
310   CONTINUE
      ...
      SUBROUTINE INIT_F1(NIND,NATT,NG,XSTAN,XK,GRP,N,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
       INCLUDE 'emmix/EMMIX-f1.max'
      DIMENSION XK(MAXNG,MNATT),
     &          XSTAN(MNATT)
...
440       CONTINUE
      ...
      SUBROUTINE UPDATE_F1(NIND,NATT,NG,XSTAN,XK,GRPOLD,GRP,N,IER)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
       INCLUDE 'emmix/EMMIX-f1.max'
...
440       CONTINUE
      ...
      FUNCTION RULE_F1(NG,NATT,XKOLD,XK)
...
510   CONTINUE
      ...
      SUBROUTINE FACTE_F1(NIND,NATT,NG,X,W,GAMM,XMU,NATTQ)
...
300   CONTINUE
      ...
      SUBROUTINE FSAMSP_F1(NIND,NATT,NG,NCOV,XVAR,V2,D,B,
...
C	CALL VMULT(V2,0,B,0,NATT,NATT,NATTQ,NG,GAMM)       !inv(SigmaN)*B
	DO 10 L=1,NATTQ
	  DO 10 I=1,NATT
            GAMM(K,I,L)=0.0D0
 	    DO 10 J=1,NATT
	      GAMM(K,I,L)=GAMM(K,I,L)+V2(K,IC1(I,J))*B(K,J,L)
10     CONTINUE
C--------------------------------------------------------------
C      Calculate Alpha
C--------------------------------------------------------------
C	CALL MULT(B,1,V2,0,NATTQ,NATT,NATT,NG,TEMP1)       ! (B'*SigmaN
	DO 20 L=1,NATT
	  DO 20 I=1,NATTQ
            TEMP1(L,I)=0.0D0
 	    DO 20 J=1,NATT
	      TEMP1(L,I)=TEMP1(L,I)+B(K,J,I)*V2(K,IC1(J,L))
20     CONTINUE
C--------------------------------------------------------------
C	CALL MULT(ALPA,0,B,0,NATTQ,NATT,NATTQ,NG,TEMP)    !  *B)
	DO 30 L=1,NATTQ
	  DO 30 I=1,NATTQ
            ALPA(IC1(I,L))=0.0D0
 	    DO 30 J=1,NATT
	      ALPA(IC1(I,L))=ALPA(IC1(I,L))+TEMP1(J,I)*B(K,J,L)
30     CONTINUE
...
C	CALL EYEmA(TEMP,NATTQ,NG,ALPA)                    ! I- ^
	DO 40 I=1,NATTQ
	 DO 40 J=1,I
	   IF (I.EQ.J) THEN
	    ALPA(IC1(I,J))=1.0D0-ALPA(IC1(I,J))
	   ELSE
	    ALPA(IC1(I,J))=-ALPA(IC1(I,J))
	   ENDIF
40      CONTINUE
C--------------------------------------------------------------
...
C	CALL MULT(GAMM,1,XVAR,0,NATTQ,NATT,NATT,NG,B)  !(Gamm'*Sigma
	DO 50 L=1,NATT
	  DO 50 I=1,NATTQ
            B(K,L,I)=0.0D0
 	    DO 50 J=1,NATT
	      B(K,L,I)=B(K,L,I)+GAMM(K,J,I)*XVAR(K,IC1(J,L))
50     CONTINUE
C--------------------------------------------------------------
C	CALL MULT(B,0,GAMM,0,NATTQ,NATT,NATTQ,NG,TEMP)    ! *Gamm)
	DO 55 L=1,NATTQ
	  DO 55 I=1,NATTQ
            TEMP2(IC1(I,L))=0.0D0
 	    DO 55 J=1,NATT
	      TEMP2(IC1(I,L))=TEMP2(IC1(I,L))+B(K,J,I)*GAMM(K,J,L)
55     CONTINUE
C--------------------------------------------------------------
C	CALL ADD(TEMP,0,ALPA,0,NATTQ,NATTQ,NG,B)             ! +alpha
	DO 60 I=1,NATTQ
	  DO 60 J=1,I
            TEMP2(IC1(I,J))=TEMP2(IC1(I,J))+ALPA(IC1(I,J))
60      CONTINUE
C--------------------------------------------------------------
C       Invert result
	  IER=0
	  CALL GDETQ_F1(NATTQ,TEMP2,TEMP3,DV,IER)
	  IF (IER.GT.0) THEN
...
C	CALL MULT(TEMP,0,GAMM,1,NATTQ,NATTQ,NATT,NG,B)   ! ^ *Gamm'
        DO 70 L=1,NATT
         DO 70 I=1,NATTQ
          TEMP1(L,I)=0.0D0
          DO 70 J=1,NATTQ
           TEMP1(L,I)=TEMP1(L,I)+TEMP3(IC1(I,J))*GAMM(K,L,J)
70      CONTINUE
C--------------------------------------------------------------
C	CALL MULT(B,0,XVAR,0,NATTQ,NATT,NATT,NG,TEMP)     ! *Sigma
        DO 80 L=1,NATTQ
         DO 80 I=1,NATT
          B(K,I,L)=0.0D0
 	  DO 80 J=1,NATT
           B(K,I,L)=B(K,I,L)+XVAR(K,IC1(J,I))*TEMP1(J,L)
80      CONTINUE
...
C	CALL MULT(XVAR,0,GAMM,0,NATT,NATT,NATTQ,NG,TEMP)  ! (Sigma*Gamm
        DO 90 L=1,NATTQ
         DO 90 I=1,NATT
          TEMP1(I,L)=0.0D0
	  DO 90 J=1,NATT
           TEMP1(I,L)=TEMP1(I,L)+XVAR(K,IC1(I,J))*GAMM(K,J,L)
90      CONTINUE
C--------------------------------------------------------------
...
C       CALL MULT2V(TEMP2,1,B,0,NATTQ,NATT,NATTQ,NG,V2)    !   ^ * B
	DO 265 L=1,NATTQ
	   ...
	   TEMP2(IC1(L,I))=0.0D0
           DO 265 J=1,NATT
            TEMP2(IC1(L,I))=TEMP2(IC1(L,I))+TEMP1(J,I)*B(K,J,L)
265     CONTINUE
C--------------------------------------------------------------
C	CALL EYEaA(V2,NATTQ,NG,TEMP)                  ! I + ^
        DO 266 I=1,NATTQ
	   TEMP2(IC1(I,I))=1.0D0+TEMP2(IC1(I,I))
266     CONTINUE
C--------------------------------------------------------------
	  IER=0
	  CALL GDETQ_F1(NATTQ,TEMP2,TEMP3,DV,IER)
...
C	  CALL MULT(BD,1,V2,0,NATT,NATTQ,NATT,NG,TEMP)
	  DO 120 I=1,NATT
	    ...
	    TEMP4(I,L)=0.0D0
	    DO 120 J=1,NATTQ
CC	     TEMP4(I,L)=TEMP4(I,L)+TEMP1(J,I)*V2(K,IC1(J,L))
	     TEMP4(I,L)=TEMP4(I,L)+TEMP1(I,J)*TEMP3(IC1(J,L))
120     CONTINUE
C--------------------------------------------------------------
C	  CALL MULT(TEMP,0,BD,0,NATT,NATTQ,NATT,NG,V2)
	  DO 130 I=1,NATT
	    ...
	    V2(K,IC1(I,L))=0.0D0
	    DO 130 J=1,NATTQ
	     V2(K,IC1(I,L))=V2(K,IC1(I,L))+TEMP4(I,J)*TEMP1(L,J)
130     CONTINUE
C--------------------------------------------------------------
	  DO 270 I=1,NATT
	   DO 270 J=1,I
	    IF (I.EQ.J) THEN
	     V2(K,IC1(I,J))=1.0/D(K,J)-V2(K,IC1(I,J))
	    ELSE
	     V2(K,IC1(I,J))=-1*V2(K,IC1(I,J))
	    ENDIF
270       CONTINUE
C--------------------------------------------------------------
...
C	  CALL MULT(BD,0,B,0,NATTQ,NATT,NATTQ,NG,TEMP)  !B'*inv(B'B+D)*B
	  DO 150 I=1,NATTQ
	    ...
	    TEMP2(IC1(I,L))=0.0D0
	    DO 150 J=1,NATT
	     TEMP2(IC1(I,L))=TEMP2(IC1(I,L))+TEMP1(J,I)*B(K,J,L)
150     CONTINUE
C--------------------------------------------------------------
C	  CALL EYEmA(TEMP,NATTQ,NG,BD)               !I-B'*inv(B'B+D)*B
	  DO 170 I=1,NATTQ
	   DO 160 J=1,I
	    TEMP2(IC1(I,J))=-TEMP2(IC1(I,J))
160        CONTINUE
	   TEMP2(IC1(I,I))=1.0+TEMP2(IC1(I,I))
170       CONTINUE
C--------------------------------------------------------------
	  IER=0
...
2001    CONTINUE
      ...
      SUBROUTINE FSASET2_F1(NIND,NATT,NG,NCOV,X,DV2,XVAR,V2,
     &                  D,B,XMU,NATTQ,MODE,FACT,SETUPON,T,IER)
C     This Subroutine implements the M-step of the EM algorithm
      implicit double precision (a-h,o-z)
       INCLUDE 'emmix/EMMIX-f1.max'
...
C     Initialse method by B=0 and D= diag(component covariances)
	...
	CALL GDET_F1(NCOV,NATT,NG,XVAR,V2,DV2,IER)
        DPROD=1
        DO 1200 K=1,NG
 	 DO 1110 I=1,NATT
	  D(K,I)=V2(K,IC1(I,I))
...
C--------------------------------------------------------------
	 DO 100 I=1,NATT
	  DO 100 J=1,I
	   V2(K,IC1(I,J))=XVAR(K,IC1(I,J))*SQRT((1.0/D(K,I))*(1.0/D(K,J)))
100      CONTINUE
C--------------------------------------------------------------
...
C	CALL MULT2V(TEMP2,1,B,0,NATTQ,NATT,NATTQ,NG,V2)    !   ^ * B
 	DO 265 L=1,NATTQ
         DO 265 I=1,L
          TEMP5(IC1(L,I))=0.0D0
          DO 265 J=1,NATT
	   TEMP5(IC1(L,I))=TEMP5(IC1(L,I))+TEMP2(J,I)*B(K,J,L)
265     CONTINUE
C--------------------------------------------------------------
C	CALL EYEaV(V2,NATTQ,NG,TEMP5)                  ! I + ^
	DO 266 I=1,NATTQ
           TEMP5(IC1(I,I))=1.0D0+TEMP5(IC1(I,I))
266     CONTINUE
C--------------------------------------------------------------
	  IER=0
	  CALL GDETQ_F1(NATTQ,TEMP5,TEMP3,DV,IER)
	  IF (IER.GT.0) THEN
	   WRITE (*,*) 'ERROR inverting Sigma New matrix in Mstep'
	   RETURN
	  ENDIF
...
C	  CALL MULTV(TEMP2,0,TEMP3,0,NATT,NATTQ,NATTQ,TEMP4)
          DO 267 L=1,NATTQ
           DO 267 I=1,NATT
            TEMP4(I,L)=0.0D0
            DO 267 J=1,NATTQ
             TEMP4(I,L)=TEMP4(I,L)+TEMP2(I,J)*TEMP3(IC1(J,L))
267     CONTINUE
C--------------------------------------------------------------
...
C	  CALL VMULT(TEMP4,0,TEMP2,1,NATT,NATTQ,NATT,NG,V2)****
	DO 268 L=1,NATT
          DO 268 I=1,NATT
            V2(K,IC1(I,L))=0.0D0
            DO 268 J=1,NATTQ
	     V2(K,IC1(I,L))=V2(K,IC1(I,L))+TEMP4(I,J)*TEMP2(L,J)
268     CONTINUE
C--------------------------------------------------------------
	  DO 270 I=1,NATT
	   DO 270 J=1,I
...
C	  CALL MULTV(V2,1,B,0,NATT,NATT,NATTQ,NG,TEMP2)    !B'*(B'B+D)
	   DO 271 L=1,NATTQ
            DO 271 I=1,NATT
             TEMP2(I,L)=0.0D0
             DO 271 J=1,NATT
              TEMP2(I,L)=TEMP2(I,L)+V2(K,IC1(I,J))*B(K,J,L)
271       CONTINUE
C--------------------------------------------------------------
...
C	  CALL VMULT(TEMP2,1,B,0,NATTQ,NATT,NATT,NG,TEMP3)  !B'*(B'B+D)*B
	   DO 272 L=1,NATTQ
            DO 272 I=1,NATTQ
             TEMP3(IC1(I,L))=0.0D0
             DO 272 J=1,NATT
              TEMP3(IC1(I,L))=TEMP3(IC1(I,L))+TEMP2(J,I)*B(K,J,L)
272       CONTINUE
C--------------------------------------------------------------
...
C	  CALL EYEmV(TEMP3,NATTQ,NG,TEMP5)               !I-B'*(B'B+D)*B
	  DO 273 I=1,NATTQ
	   DO 273 J=1,I
	    IF (I.EQ.J) THEN
	     TEMP3(IC1(I,J))=1.0-TEMP3(IC1(I,J))
	    ELSE
	     TEMP3(IC1(I,J))=-1*TEMP3(IC1(I,J))
	    ENDIF
273       CONTINUE
C--------------------------------------------------------------
	  IER=0

	  CALL GDETQ_F1(NATTQ,TEMP3,TEMP5,DV,IER)
	  DV2(K)=DV
	  IF (IER.GT.0) THEN
	   WRITE (*,*) 'ERROR Calculating det of Sigma New'
	   RETURN
...
c         CALL FSASET2_F1(NIND,NATT,NG,NCOV,X,XVAR,DV,DV2,V2,D,B,
c     &               XMU,NATTQ,T,IER)
c
...
c       CALL ESTEP_F1(NIND,NATT,NG,X,XMU,V2,T,DEN,WL,W,XUU,USA,DV2,
c     &            XLOGL,IOUNT,XMAH,IER)
cC       RE-Calculate Mu and Pi and XVAR
c      CALL MSTEP_F1(NIND,NATT,NG,NCOV,X,XVAR,V,DV,
c     &           XMU,WTOT,T,W,XUU,XMAH,TEMP,U,IER)
c       ELSE
c       CALL ESTEP_F1(NIND,NATT,NG,X,XMU,V,T,DEN,WL,W,XUU,USA,DV,
c     &            XLOGL,IOUNT,XMAH,IER)
c       ENDIF
c
c      IF ((FLAGS(22).EQ.100).OR.(FLAGS(23).EQ.2)) THEN
c        IER=0
c        CALL FSAMSP_F1(NIND,NATT,NG,NCOV,XVAR,V2,D,B,
...
c     &           XMU,WTOT,T,W,XUU,XMAH,TEMP,U,IER)
cccc      ENDIF
      SUBROUTINE TDIAG_F1(N,KK,A,D,E,Z,IFAULT)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
C
       INCLUDE 'emmix/EMMIX-f1.max'
