...
less available
recreate using Bioconductor
basic processing with vsn for purpose of example
...
alon_data<-(Biobase::exprs(vsn::vsn2(colonCA)))[1:1000,]
golub_data<-(Biobase::exprs(vsn::vsn2(Golub_Merge)))[1:1000,]
...
see tha link for issue on datasets being different

different scale too


```{r, fig.show='hold', cache=T}
plot_single_gene(alon_data,201, min =6, max=14)
plot_single_gene(alon_data,202,g=3, min =6, max=14)
```



```{r, fig.show='hold', cache=T}
plot_single_gene(alon_data, 202, g=2, min =6, max=14)
```

## Select Genes

Slecting genes as in section
...
golub_clus_10_t3<-cluster_tissues(golub_sel, golub_clus_10, method='mfa', q=1)
```
